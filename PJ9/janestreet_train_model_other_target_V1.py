...
V5 :
    Started from V3 code
    Reusing models without gamma from "21/12 matin"
    Choice between classifier (predict resp > 0 boolean) and regressor (predict resp, then outputs boolean resp prediction > 0) model

V6 :
    run1 :
        Add some feature engineering test: feature_0 * other features
    
    2nd run : 
        only features x0  and utility 5,4
        
    3rd run:
        normal features + feature x0 but only : 
                ['feature_113_x0', 'feature_108_x0' , 'feature_45_x0', 'feature_44_x0' , 'feature_41_x0' , 'feature_42_x0' , 'feature_63_x0' , 'feature_43_x0' , 'feature_62_x0', 'feature_60_x0' , 'feature_61_x0' , 'feature_6_x0' , 'feature_84_x0', 'feature_4_x0' , 'feature_6_x0' , 'feature_83_x0' , 'feature_38_x0' , 'feature_5_x0', 'feature_92_x0', 'feature_119_x0']


train_model_other_target_V1:
    Train a model to predict resp n-1 (this will be used as base model in janestreet_train_ensemble script)
...
LOGFILE = 'janestreet_train_model_other_target.log'
...
if (FEATURE_ENGINEERING == True):
    #FEATURES_LIST_TOTRAIN = ['feature_'+str(i) for i in range(130)] + ['feature_'+str(i)+'_x0' for i in range(130)] + ['weight', 'weight_x0']
    #FEATURES_LIST_TOTRAIN = ['feature_'+str(i)+'_x0' for i in range(130)] + ['weight_x0']
    FEATURES_LIST_TOTRAIN = ['feature_'+str(i) for i in range(130)] + ['feature_113_x0', 'feature_108_x0' , 'feature_45_x0', 'feature_44_x0' , 'feature_41_x0' , 'feature_42_x0' , 'feature_63_x0' , 'feature_43_x0' , 'feature_62_x0', 'feature_60_x0' , 'feature_61_x0' , 'feature_6_x0' , 'feature_84_x0', 'feature_4_x0' , 'feature_6_x0' , 'feature_83_x0' , 'feature_38_x0' , 'feature_5_x0', 'feature_92_x0', 'feature_119_x0']
...
class XGBClassifier_wrapper(BaseEstimator, ClassifierMixin):  
    ...
    def __init__(self, params):
        ...
        if (MODEL_TYPE == 'REGRESSION'):
            self.model_internal = XGBRegressor(
    ...
    def predict(self, X, y=None):
        if (self.fitted == True):
            ...
            if (MODEL_TYPE == "REGRESSION"):
                y_pred = ((self.model_internal.predict(X[self.features])) > 0).astype(np.byte)
                
            else:
                y_pred = self.model_internal.predict(X[self.features])
    ...
    def predict_proba(self, X, y=None): # Not tested in regression mode: probably won't work
        if (self.fitted == True):
...
def hyperopt_train_test(params):
    ...
    for train_index, test_index in cv.split(df, (df['resp'] > 0)*1, df['date']):
        ...
        if (MODEL_TYPE == 'REGRESSION'): # Not implemented  in this script : does not work
            model_wrapped.fit(df.loc[train_index], df.loc[train_index]['resp'])
            
        else:
            model_wrapped.fit(df.loc[train_index], (df.loc[train_index]['resp'] > 0).astype(np.byte).shift(1, fill_value=0))        
        
        test_predictions = model_wrapped.predict(df.loc[test_index])
        scores.append(model_wrapped.score(df.loc[test_index], test_predictions)) # We use resp n-1 prediction to predict resp : scores might be poor in that case but it's ok because it's only a base model
...
'''
    {
   'features': ['feature_'+str(i) for i in range(130)] + ['feature_'+str(i)+'_x0' for i in range(130)] + ['weight', 'weight_x0'], 
    'random_state': 42,
    'max_depth': 10,
    'n_estimators': 500,
    'learning_rate': 0.01,
...
if (FEATURE_ENGINEERING == True):
    for i in range(130):
        df['feature_'+str(i)+'_x0'] = df['feature_0'] * df['feature_'+str(i)]
    
    df['weight_x0'] = df['feature_0'] * df['weight']
...
for params in iteration_params:
    ...
    if (TRAIN_PERIMETER_ALL == True):
        ...
        if (MODEL_TYPE == 'REGRESSION'): # Not implemented
            model_wrapped.fit(df, df['resp'])
            
        else:
            model_wrapped.fit(df, (df['resp'] > 0).astype(np.byte).shift(1, fill_value=0))
