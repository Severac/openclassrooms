...
V5 bis:
    Same as V5 but predict resp*weight instead of resp, and include weight in features
...
def hyperopt_train_test(params):
    ...
    for train_index, test_index in cv.split(df, (df['resp'] > 0)*1, df['date']):
        ...
        if (MODEL_TYPE == 'REGRESSION'):
            model_wrapped.fit(df.loc[train_index], df.loc[train_index]['resp']*df.loc[train_index]['weight'])
...
iteration_params = [
       {
   'features': ['feature_'+str(i) for i in range(130)] + ['weight'], 
    'random_state': 42,
    'max_depth': 10,
    'n_estimators': 500,
    'learning_rate': 0.02,
...
for params in iteration_params:
    ...
    if (TRAIN_PERIMETER_ALL == True):
        ...
        if (MODEL_TYPE == 'REGRESSION'):
            model_wrapped.fit(df, df['resp']*df['weight'])
