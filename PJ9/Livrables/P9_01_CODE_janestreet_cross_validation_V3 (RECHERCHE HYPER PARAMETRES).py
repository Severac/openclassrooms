...
# This version focuses on tunning gamma parameter after having tuned other
# /!\ This version has different hyper parameters than V2, so it's incompatible with resuming from trials object of V2
# Under construction
...
'''
params_space_old = {  # This was used in V2 script
...
params_space = {  
   'features': hp.choice('features', [['feature_'+str(i) for i in range(130)]]), 
    ...
    'n_estimators': hp.choice('n_estimators', [250, 500]),
    'learning_rate': hp.choice('learning_rate', [0.01, 0.02, 0.1]),
    'subsample': hp.choice('subsample', [0.5, 0.8]),
    'colsample_bytree': hp.choice('colsample_bytree', [0.2, 0.3, 0.6, 0.9]),
    'gamma': hp.choice('gamma', [0.01, 0.1, 0.5, 1, 10]),
    'tree_method': hp.choice('tree_method', ['gpu_hist'])
}
...
already_trained = {} # We save already trained params with their scores so that we won't run optimisation again on them

def f(params):
    print('New call of f')
    print('Training with params: ' + str(params))
    
    if (already_trained.get(str(params), False) != False):
        print('A training has already been done with those params ! So we directly use previous loss result')
        scores_obj = {'utility_score': already_trained[str(params)], 'utility_scores': None, 'utility_score_std': None, 'accuracy_scores': None}
    
    else:
        scores_obj = hyperopt_train_test(params)
    
    already_trained[str(params)] = scores_obj['utility_score']
