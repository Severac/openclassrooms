...
V3 different data:
    Best model of 21/12 trained on :
        Data with first days removed
        Only data of splits 4 and 5
        Data without weight 0
...
for n_data_iteration in range(n_data_iterations):
    # Load data
    print('---------------------------------------')
    print(f'Data iteration {n_data_iteration} : loading data')
    
    df = pd.read_csv(DATASET_INPUT_FILE)
    df['resp_positive'] = ((df['resp'])>0)*1  # Target to predict
    
    if (n_data_iteration == 0):
        print('Removing all dates <= 85)')
        #df = df[df['date'] > 85].reset_index(drop=True)
        df.drop(df[df['date'] <= 85].index, axis=0, inplace=True)
        df.reset_index(drop=True, inplace=True)
        
    elif (n_data_iteration == 1):
        print('Keeping only samples from split 4 and 5')
        #df = df.iloc[877453:].reset_index(drop=True)
        df.drop(df.iloc[0:877453].index, axis=0, inplace=True) # See kaggle notebook "Jane Street proba cross validation" for list of split indices, to find value of 877453
        df.reset_index(drop=True, inplace=True)

    elif (n_data_iteration == 2):
        print('Keeping only weight different than 0')
        #df = df[df['weight'] != 0].reset_index(drop=True)
        df.drop(df[df['weight'] == 0].index, axis=0, inplace=True)
        df.reset_index(drop=True, inplace=True)
        
    elif (n_data_iteration == 3):
        print('Keeping only weight different than 0 AND samples from split 4 and 5')
        ...
        # Note : 698278 is the first indice of date 180 (which is the first date of split 4) after having dropped weight == 0
    ...
    for params in iteration_params:
        ...
        if (TRAIN_PERIMETER_ALL == True):
            ...
            print('Save model')
            model_wrapped.model_internal.save_model(MODEL_FILE + '_' + iteration_names[n] + '_dataiteration' + str(n_data_iteration))
