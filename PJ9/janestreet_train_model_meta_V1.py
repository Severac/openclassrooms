...
janestreet_train_model_meta_V1.py:
...
def hyperopt_train_test(params):
    ...
    for train_index, test_index in cv.split(df, (df['resp'] > 0)*1, df['date']):
        ...
        # Shit values of resp to get resp of step n-1
        y_train_resp_n1_positive = y_train_resp_positive.shift(1, fill_value=0)
        ...
        train_predictions_resp_n1_proba = model_n1.predict_proba(df.loc[train_index, FEATURES_LIST_TOTRAIN])
        df.loc[train_index, 'resp_n1_predict'] = pd.DataFrame(train_predictions_resp_n1_proba, index=train_index)[1].apply(resp_n1_value)
        
        model_wrapped.fit(df.loc[train_index], (df.loc[train_index]['resp'] > 0).astype(np.byte))
        
        test_predictions_resp_n1_proba = model_n1.predict_proba(df.loc[test_index, FEATURES_LIST_TOTRAIN])
        df.loc[test_index, 'resp_n1_predict'] = pd.DataFrame(test_predictions_resp_n1_proba, index=test_index)[1].apply(resp_n1_value)
        ...
        print('Comptage des valeurs de resp n1 predict:')
        print(df.loc[test_index, 'resp_n1_predict'].value_counts())
        ...
        precision_n1 = precision_score((df.loc[test_index]['resp'] > 0).astype(np.byte).shift(1, fill_value=0), df.loc[test_index, 'resp_n1_predict'], average='macro', labels=[1])
        recall_n1 = recall_score((df.loc[test_index]['resp'] > 0).astype(np.byte).shift(1, fill_value=0), df.loc[test_index, 'resp_n1_predict'], average='macro', labels=[1])
...
iteration_names = ['meta1_from_bestutility4']
iteration_params = [
...
for params in iteration_params:
    ...
    if (TRAIN_PERIMETER_ALL == True):
        ...
        # Calculate label of current step
        y_train_resp_positive = (df['resp'] > 0).astype(np.byte)
        ...
        model_n1.fit(df[FEATURES_LIST_TOTRAIN], y_train_resp_n1_positive, verbose=True)
        
        train_predictions_resp_n1_proba = model_n1.predict_proba(df[FEATURES_LIST_TOTRAIN])
        df['resp_n1_predict'] = pd.DataFrame(train_predictions_resp_n1_proba, index=df.index)[1].apply(resp_n1_value)
        ...
        print('Save model resp_n1')
        model_n1.save_model(MODEL_FILE + iteration_names[n] + '_resp_n1')
