...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan  1 01:16:27 2021

@author: francois

V1 : load models as features and do logistic regression

V2 : same thing with keras model (bottleneck from public notebook) as base model
"""
...
MODEL_INPUT_FILES = [
        #{'path' : '/home/francois/coding/OC/PJ9/trainings/20-12_soir/model.binbest_utility4', 
        {'path' : '/home/ubuntu/janestreet/train_bck/model.binbest_utility4',
         'type' : 'XGBClassifier'},
        #{'path':  '/home/francois/coding/OC/PJ9/trainings/25-12_regression/model.binbest_utility5_withoutgamma',
        {'path': '/home/ubuntu/janestreet/train_bck/train_bck_other_target/model.binresp_n1',
          'type': 'XGBClassifier'},]

MODEL_FILE = 'model_ensemble.bin'

LOGFILE = 'janestreet_train_model_ensemble.log'
...
print('Start of training ensemble script. Loading data...')
...
class LogisticRegression_wrapper(BaseEstimator, ClassifierMixin):  
    def __init__(self):
        self.fitted = False
        
        self.model_internal = LogisticRegression()

    def fit(self, X, y=None):
        print('Model used for fitting:')
        print(self.model_internal)
        self.model_internal.fit(X, y)
    ...
    def predict(self, X, y=None):
        if (self.fitted == True):
            print('predict called')
            return(self.model_internal.predict(X))
    ...
    def predict_proba(self, X, y=None):
        if (self.fitted == True):
            print('predict proba called')
            return(self.model_internal.predict_proba(X))
    ...
    def score(self, X, y=None):        
        ...
        if y is None:
            print('y is None')
            y_preds = pd.Series(self.model_internal.predict(X.reset_index(drop=True)))
...
def hyperopt_train_test(params):
    ...
    model_wrapped = LogisticRegression_wrapper()
    
    models = []
    for model_attributes in params:
        if (model_attributes['type'] == 'XGBClassifier'):
            model = XGBClassifier()
            model.load_model(model_attributes['path'])
            models.append(model)
        
        elif (model_attributes['type'] == 'XGBRegressor'):
            model = XGBRegressor()
            model.load_model(model_attributes['path'])
            models.append(model)            
    ...
    for train_index, test_index in cv.split(df, (df['resp'] > 0)*1, df['date']):
        ...
        df_base_input_train = pd.DataFrame()
        df_base_input_test = pd.DataFrame()
        
        model_indice = 0
        for model, model_attributes in zip(models, params):            
            if (model_attributes['type'] == 'XGBClassifier'):            
                df_base_input_train['model_'+str(model_indice)] = model.predict_proba(df.loc[train_index, FEATURES_LIST_TOTRAIN])[:, 1]
                df_base_input_test['model_'+str(model_indice)] = model.predict_proba(df.loc[test_index, FEATURES_LIST_TOTRAIN])[:, 1]
            
            elif (model_attributes['type'] == 'XGBRegressor'):  # Does not work currently (only prediction via predict_proba is implemented)              
                df_base_input_train['model_'+str(model_indice)] = (model.predict(df.loc[train_index, FEATURES_LIST_TOTRAIN]) > 0).astype(np.byte)
                df_base_input_test['model_'+str(model_indice)] = (model.predict(df.loc[test_index, FEATURES_LIST_TOTRAIN]) > 0).astype(np.byte)

            else:
                print('! Warning : unknown model type:')
                print(model_attributes['type'])
            
            print(f'Accuracy score of model {model_indice} on training set:')
            print(accuracy_score((df.loc[train_index]['resp'] > 0).astype(np.byte), (df_base_input_train['model_'+str(model_indice)] > 0.5).astype(np.byte)  ))

            print(f'Accuracy score of model {model_indice} on test set:')
            print(accuracy_score((df.loc[test_index]['resp'] > 0).astype(np.byte), (df_base_input_test['model_'+str(model_indice)] > 0.5).astype(np.byte)  ))

            print(f'Utility score of model {model_indice} on test set:')
            print(utility_function_normalized(df.loc[test_index].reset_index(drop=True), (df_base_input_test['model_'+str(model_indice)] > 0.5).astype(np.byte) ))
            
            model_indice += 1                   
        
        model_wrapped.fit(df_base_input_train, (df.loc[train_index]['resp'] > 0).astype(np.byte))
        
        test_predictions = model_wrapped.predict(df_base_input_test)
        ...
        #No feature importance for logistic regression, so print coefs instead
        ...
        print(f'Coefs for split {i}:')
        print(model_wrapped.model_internal.coef_)
        print(f'Classes for split {i}:')
        print(model_wrapped.model_internal.classes_)
...
iteration_names = ['classifier_regressor_stacking']
iteration_params = [MODEL_INPUT_FILES]
    ...
    if (TRAIN_PERIMETER_ALL == True):
        ...
        for model_attributes in params:
            if (model_attributes['type'] == 'XGBClassifier'):
                ...
                df_base_input_train['model_'+str(model_indice)] = model.predict_proba(df.loc[:, FEATURES_LIST_TOTRAIN])[:, 1]
                df_base_input_test['model_'+str(model_indice)] = model.predict_proba(df.loc[:, FEATURES_LIST_TOTRAIN])[:, 1]
            
            elif (model_attributes['type'] == 'XGBRegressor'): # not implemented, does not work
                model = XGBRegressor()
                model.load_model(model_attributes['path'])
                models.append(model)            

                df_base_input_train['model_'+str(model_indice)] = (model.predict(df.loc[:, FEATURES_LIST_TOTRAIN]) > 0).astype(np.byte)
                df_base_input_test['model_'+str(model_indice)] = (model.predict(df.loc[:, FEATURES_LIST_TOTRAIN]) > 0).astype(np.byte)
        ...
        model_wrapped_final = LogisticRegression_wrapper()
        model_wrapped_final.fit(df_base_input_train, (df['resp'] > 0).astype(np.byte))
                
        print('Save final (meta) model')
        with open(MODEL_FILE + iteration_names[n] + '.pickle', 'wb') as file:
            pickle.dump(model_wrapped_final.model_internal, file)
        
        #model_wrapped_final.model_internal.save_model(MODEL_FILE + iteration_names[n] )
                
        print(f'Coefs final model:')
        print(model_wrapped_final.model_internal.coef_)
        print(f'Classes final model:')
        print(model_wrapped_final.model_internal.classes_)
