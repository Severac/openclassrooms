...
def hyperopt_train_test(params):
    ...
    for train_index, val_index, test_index in cv.split(df, (df['resp'] > 0)*1, df['date']):
        ...
        precision_n1 = precision_score((df.loc[val_index]['resp'] > 0).astype(np.byte).shift(1, fill_value=0), df.loc[val_index, 'resp_n1_predict'])
        recall_n1 = recall_score((df.loc[val_index]['resp'] > 0).astype(np.byte).shift(1, fill_value=0), df.loc[val_index, 'resp_n1_predict'])
...
for params in iteration_params:
    ...
    if (TRAIN_PERIMETER_ALL == True):
        ...
        model_n1.fit(df[FEATURES_LIST_TOTRAIN], (df['resp'] > 0).astype(np.byte).shift(1, fill_value=0), verbose=True)
