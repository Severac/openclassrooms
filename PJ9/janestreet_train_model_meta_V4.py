...
Meta V3 :
    Use of regressor (best_utility5_withoutgamma from janestreet_train_model_V5) as a base model
    
Meta V4:
    Use of clusterer as base model feature. NOT DONE, CODE YET TO DO.
"""
...
def hyperopt_train_test(params):
    ...
    for train_index, val_index, test_index in cv.split(df, (df['resp'] > 0)*1, df['date']):
        ...
        model_base.fit(df.loc[train_index, FEATURES_LIST_TOTRAIN], df.loc[train_index]['resp'], verbose=True)
        
        val_predictions_base = model_base.predict(df.loc[val_index, FEATURES_LIST_TOTRAIN])
        df.loc[val_index, 'base_predict'] = pd.DataFrame(val_predictions_base, index=val_index, columns=['base_predict'])
        ...
        print('Count values of base predict:')
        print(df.loc[val_index, 'base_predict'].value_counts())
        ...
        precision_base = precision_score((df.loc[val_index]['resp'] > 0).astype(np.byte), (df.loc[val_index, 'base_predict'] > 0).astype(np.byte))
        recall_base = recall_score((df.loc[val_index]['resp'] > 0).astype(np.byte), (df.loc[val_index, 'base_predict'] > 0).astype(np.byte))
        precision_base_scores.append(precision_base)
        recall_base_scores.append(recall_base)
        print(f'Precision/Recall of base for split {i}:')
        print(f'Precision score for base: {precision_base}')
        print(f'Recall score for base: {recall_base}')
        
        test_predictions_base = model_base.predict(df.loc[test_index, FEATURES_LIST_TOTRAIN])
        df.loc[test_index, 'base_predict'] = pd.DataFrame(test_predictions_base, index=test_index, columns=['base_predict'])
...
iteration_names = ['meta3_from_bestutility5', 'meta3_from_bestutility4', 'meta3_from_bestutility4_with_gamma']
iteration_params = [
{
   'features': ['feature_'+str(i) for i in range(130)] + ['base_predict'], 
    'random_state': 42,
    'max_depth': 10,
    'n_estimators': 500,
    'learning_rate': 0.02,
...
for params in iteration_params:
    ...
    if (TRAIN_PERIMETER_ALL == True):
        ...
        print(df.loc[pd.Index(val_test_index_all_flat).drop_duplicates(keep='first'), 'base_predict'].shape)
        
        # We drop duplicates since there is on overlap between val/test sets of different splits
        # Note that at this point, df already contains 'base_predict' column for all val/test sets indices (that have been assigned on training code in hyperopt_train_test)
        df_final = df.loc[pd.Index(val_test_index_all_flat).drop_duplicates(keep='first'), :]
        
        print('Count of "base_predict > 0" in training set of final meta model:')
        (df_final['base_predict'] > 0).value_counts()
        ...
        model_base.fit(df[FEATURES_LIST_TOTRAIN], df['resp'], verbose=True)
        
        print('Save base model')
        model_base.save_model(MODEL_FILE + iteration_names[n] + '_base' )
        
        df_featimportance_base_model = pd.DataFrame(model_base.feature_importances_, index=df[FEATURES_LIST_TOTRAIN].columns, columns=['Importance']).sort_values(by='Importance', ascending=False)
