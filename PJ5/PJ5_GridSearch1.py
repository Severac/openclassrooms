...
if (RECOMPUTE_GRIDSEARCH == True):
    ...
    grid_search = GridSearchCV(model, param_grid, cv=5, verbose=10, error_score=np.nan, scoring=None) # TO CHANGE back to cv=5
    grid_search.fit(df_train)

    grid_search_res, df_grid_search_results_res = save_or_load_search_params(grid_search, 'gridsearch_BoWAlways_NCAAlways')
